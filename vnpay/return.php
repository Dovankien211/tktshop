...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT vt.*, dh.id as order_id, dh.tong_tien, dh.trang_thai_thanh_toan
    ...
    // Kiểm tra số tiền
    if (abs(parseVNPayAmount($transaction['vnp_amount']) - $vnp_Amount) > 1) {
        ...
        if ($vnp_ResponseCode === '00' && $vnp_TransactionStatus === '00') {
            ...
            try {
                ...
                $stmt = $pdo->prepare("
                    ...
                    JOIN chi_tiet_don_hang ctdh ON bsp.id = ctdh.bien_the_san_pham_id
                ...
                if ($transaction['order_id']) {
                    $order_info_stmt = $pdo->prepare("SELECT khach_hang_id FROM don_hang WHERE id = ?");
                    $order_info_stmt->execute([$transaction['order_id']]);
                    $order_data = $order_info_stmt->fetch();
                    
                    if ($order_data && $order_data['khach_hang_id']) {
                        $pdo->prepare("DELETE FROM gio_hang WHERE khach_hang_id = ?")
                            ->execute([$order_data['khach_hang_id']]);
                    }
                }
    ...
    $stmt = $pdo->prepare("
        SELECT dh.*, nd.ho_ten, nd.email, nd.so_dien_thoai,
               vt.vnp_transaction_no, vt.vnp_bank_code, vt.vnp_pay_date
        FROM don_hang dh
